...
from tools.graphics.nematics import fieldSnapshot
from matplotlib.colors import LinearSegmentedColormap


brown_rgb = (89/255, 168/255, 217/255)   # RGB for #994F00
light_blue_rgb = (127/255, 73/255, 23/255)  # RGB for #006CD1

#light_blue_rgb = (89/255, 168/255, 217/255)   # RGB for #994F00
#brown_rgb = (127/255, 73/255, 23/255)  # RGB for #006CD1

# Define a custom colormap that transitions from brown to light blue
brown_to_light_blue_cb_friendly = LinearSegmentedColormap.from_list(
    "brown_lightblue_cb", [brown_rgb, light_blue_rgb]
) # RGB for #56B4E9 (colorblind-friendly light blue)
...
def colorSelector(colorby, data):
    ...
    if colorby == "smectic":
        ...
        cm = brown_to_light_blue_cb_friendly
...
def drawRods(
    ...
    iterateRods(
        ax, cm(values), data.x, data.y, data.ex, data.ey, 0.95*data.l, data.phi, data.r
    )
    radius = fitted_radii_dict[time]
    #radius = fitted_radii_dict[-1]
...
def gridSnapshots(time: int, fitted_radii_dict, datapath: str, imgPath="", saveImg=False):  # New radius parameter
    data = extract_data(datapath, time)
